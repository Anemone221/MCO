...
/**
 * Pages whose entire IPC surface is one read: they share a file rather than
 * taking one each. When a board grows a mutation — a filter the main process
 * has to persist, an action on a row — give it its own module and move its read
 * across with it.
 */
